...
| Tool | Description |
|------|-------------|
| `oreilly_search_content` | コンテンツ検索 - 軽量サマリ + ファイル保存による遅延読み込み |
| `oreilly_ask_question` | O'Reilly Answers AI での技術 Q&A |
| `oreilly_reauthenticate` | セッション再認証 (Cookie 期限切れ時に Chrome を起動) |

### oreilly_search_content レスポンス

検索結果はローカル Markdown ファイルに全件保存され、ツールレスポンスには軽量サマリ (top 5) + ファイルパスのみを返す。詳細が必要な場合は Read ツールでファイルを読み取る。
...
| `orm-mcp://history/{id}/full` | キャッシュファイルの内容 |
...
| `debug-error` | Debug an Error | `error_message` (required), `technology` (required), `context` (optional) |
| `review-history` | Review Research History | `keyword` (optional) |
| `continue-research` | Continue Previous Research | `research_id` (required) |
