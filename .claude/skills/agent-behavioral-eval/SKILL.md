...
| キャッシュ品質シグナル | `orm-mcp://history/recent` の直近エントリの `file_path` を Read → `Total Results: N` (N > 0) | L_SQ |
...
| HTML 除去確認 | Read されたキャッシュファイルの Description に `<span>`, `<div>` 等の HTML タグが含まれない | L_CE |
...
- L_SQ キャッシュ品質シグナル (Total Results > 0): {status}
- L_CE HTML 除去確認 (Description に HTML タグなし): {status}
