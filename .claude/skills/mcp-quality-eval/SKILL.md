...
**並列化戦略**:
- 2A + D1 はメインエージェントが直接実行 (MCP ツール権限 / `task` コマンド権限の都合)
- 2B + 2C + 2D(D2,D4) は subagent で並列実行可 (全て静的チェック、Bash 不要)
...
#### D1: Capability Coverage (静的、メインエージェントで実行)

**重要**: メインエージェントが直接実行する。subagent は `task` コマンドの Bash 実行で permission denied になるため。2A の認証確認直後、`oreilly_ask_question` の待機中に D1 を実行し、ネットワーク待ちと並行させる。
...
| `agent_behavioral_lazy_loading` | Agent Quality | 遅延読み込み判断精度 (Total Results に基づく Read 要否) |
...
2. **Phase 2 は並列化**: 2A + D1 はメインで実行、2B/2C/2D(D2,D4) は subagent で同時実行 (Bash 不要の静的チェックのみ)。2A 完了後に D3 (agent-behavioral-eval) 開始。2A が FAIL (認証失敗) の場合、D3 全体を SKIP とする
